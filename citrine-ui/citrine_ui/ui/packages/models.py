...
from citrine_ui import errors, app
...
class InstalledPackage(QtCore.QObject):
    ...
    def active_toggle(self):
        client = citrine_client.api.PackageClient(get_config('daemon.server'), get_config('daemon.port'))
        try:
            if self.active:
...
class ProgressList(QtCore.QObject):
    ...
    def install_package(self, name: str):
        ...
        self.packages.append(pkg)
        self.sig_data_changed.emit()
        client = citrine_client.api.PackageClient(get_config('daemon.server'), get_config('daemon.port'), async_=True)
        try:
